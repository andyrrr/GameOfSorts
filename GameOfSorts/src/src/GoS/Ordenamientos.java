...
public class Ordenamientos {
	...
	/**
	 * Se reciba la lista de dragones para ser acomodados y el número de dragones
	 * muertos, paa saber en cuál ordenamiento se debe de enviar Se crean dos
	 * listas, una con la edad de cada dragón, por cada dragón, se le agrea la edad
	 * asignada a una nueva lista lo mismo sucede con la lista de recargas
	 * 
	...
	public void Ordenar(Lista<Dragon> listaDragones, int dragonesMatados) {
		...
		/**
		 * Estos son los ciclos de ordenamiento, cuando el ciclo termine vuelvve al
		 * principio, se cambia el String lista por la cuál fue utilizada para ordnar,
		 * se usarán 3 algoritmos de ordenamiento y dos álboles Se trabajarán con las
		 * listas en tiempo real, es decir, no se harán copias, todo se trabajará en la
		 * misma dirección de la lista original
		 */
	...
	/**
	 * El selection Sort funcionará con las edades, por lo tanto recibe la lista de
	 * edades para poder acomodalos, y a la misma vez la lista de objeto Dragón para
	 * ser modificada
	 * 
	 * @param listaDragones
	 * @param listaEdades
	...
	void selectionSort(Lista<Dragon> listaDragones, Lista<Integer> listaEdades) {
		...
		for (int i = 0; i < n - 1; i++) {
			/**
			 * Encuentra el mínimo elemento desordenado y se compara con otro hasta
			 * encontrar otro elemento desordenado
			 */
			int min_idx = i;
			for (int j = i + 1; j < n; j++)
			...
			/**
			 * Cambia el mínimo elemento encontrado con el primero dentr de mbas listas, y
			 * así se asegura de que los dragones quedan acomodados con respecto a la listas
			 * de edades
			 */
	...
	void quickSort(Lista<Dragon> listaDragones, Lista<Integer> listaEdades, int menor, int mayor) {
		if (menor < mayor) {
			/**
			 * pi es la partición de la lista en ds mitades, es la clase que hace todo el
			 * trabajo
			 */
	...
	int partitionQS(Lista<Dragon> listaDragones, Lista<Integer> listaEdades, int menor, int mayor) {
		/**
		 * Se declara un pivote, que va a ser el valor del último elemnto de la lista
		 * envida elemento
		 */
		...
		for (int j = menor; j < mayor; j++) {
			/**
			 * Se da la condición, si el valor en la posición j es menor o igual al pivote y
			 * se aumenta el contador i, que será el menor desordenado
			 */
			if (listaEdades.retornar(j) <= pivote) {
				i++;

				/**
				 * Se cambia de posición el i(posición del menor) con j (Elemento evaluado) y se
				 * cre un nodo temporal que tomará el valor de i para luego colocarlo en la
				 * posición j Se hace lo mismo con lista de dragones, se modifican posiciones y
				 * nodos (En este caso de tipo Dragon)
				 */
		...
		/**
		 * En caso que terminen de recorrer y no haya cumplido la condición, se
		 * intercambia el valor de la posición i+1 con el elemnto mayor, y se realiza lo
		 * mismo con la lista de dragones y se devuelve la posición i +1
		 */
	...
	void insertionSort(Lista<Dragon> listaDragones, Lista<Integer> listaRecarga) {
		...
		for (int i = 1; i < n; ++i) {
			...
			int j = i - 1;
			/**
			 * Mueve los elementos que son mayores que el temporal, y se repite hasta que no
			 * hayan elementos menores antes del temporal
			 */
	...
	/*
	 * public static void main(String args[]) { String lista1 =
	 * "3,60,35,2,45,320,5"; String lista2 = "7,9,24,5,233,77,68"; String lista3 =
	 * "55,3,6,1,8,95,0,47"; Lista<Integer> listaSS =
	 * Ordenamientos.convertir(lista1); Lista<Integer> listaQS =
	 * Ordenamientos.convertir(lista2); Lista<Integer> listaIS =
	 * Ordenamientos.convertir(lista3); Ordenamientos ob1 = new
	 * Ordenamientos(listaSS, 1); Ordenamientos ob2 = new Ordenamientos(listaQS, 2);
